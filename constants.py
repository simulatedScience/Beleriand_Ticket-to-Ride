...
# filename sources
TRACK_DATA_FILEPATH = os.path.join(CURRENT_PATH, 'tracks.csv')
BASE_TICKETS_FILEPATH = os.path.join(CURRENT_PATH, 'tickets.csv')
BIGCITIES_TICKETS_FILEPATH = os.path.join(CURRENT_PATH, 'tickets_bigcities.csv')
...
with open(TRACK_DATA_FILEPATH, 'r') as f:
  reader = csv.reader(f)
  next(reader)
...
TRACK_DECK = functools.reduce(
    list.__add__, [[color] * n for color, n in TRACK_DECK_LIST.items()])

DECK_COLORS = list(TRACK_DECK_LIST.keys())
DECK_COLORS.sort()
...
BASE_COLORS = ['green', 'red', 'yellow', 'black',
               'white', 'orange', 'pink', 'blue', 'gray']
BASE_COLORS.sort()
...
with open(BASE_TICKETS_FILEPATH, 'r') as f:
  reader = csv.reader(f)
  next(reader)
  for i, row in enumerate(reader):
    city1 = row[0]
...
with open(BIGCITIES_TICKETS_FILEPATH, 'r') as f:
  reader = csv.reader(f)
  next(reader)
  for i, row in enumerate(reader):
    city1 = row[0]
