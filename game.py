...
class Game(object):
  ...
  def calculate_end_game(self):
    ...
    if len(first_win_check) == 1:
      ...
      for player in first_win_check:
        max_completed_tickets = max(
            max_completed_tickets, len(player.completed_tickets))
      second_win_check = []
      for player in first_win_check:
  ...
  def __str__(self):
    winners = str(
        [i for i, player in enumerate(self.players) if player.win])
    return f"{self.n_players}-player game on turn {self.turn}; winners: {winners}"
  ...
  def check_if_too_many_locomotives(self):
    ...
    """
    n_locomotives = len(
        [0 for color in self.face_up_trains if color == 'locomotive'])
  ...
  def serialize_track_segment(self, segment):
    ...
    if segment['double']:
      ...
      if segment['occupied2']:
        track_ownership_vector[segment['occupied2'] +
                               self.n_players] = 1
  ...
  def calc_serialized_board_state(self, player=None):
    ...
    if not hasattr(self, 'serialized_board_state'):
      self.serialized_board_state = np.zeros(
          N_TRACKS * self.n_players * 2)
    if player == None:
  ...
  def serialize_deck_state(self):
    ...
    for card in self.face_up_trains:
      # DECK_COLOR_INDEXES[color]
      color_vector[DECK_COLOR_INDEXES[card]] += 1
    if not hasattr(self, 'discard_vector'):
      self.discard_vector = np.zeros(9)
    cards_remaining_vector = len(
        self.trains) / 50.  # 50 makes the number easier on the network regularizers
