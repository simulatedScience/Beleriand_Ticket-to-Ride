...
class Player(object):
  def __init__(self, n_players, game, pre_existing_ai=None, id=None, memory=0, q_lag=8, separated_models=True):
    ...
    self.SERIALIZATION_LENGTH = 2 * n_players * \
        N_TRACKS + 19 + 24 + 14 * (n_players - 1)
    self.game = game
    self.id = id
    ...
    self.base_ticket_serialization = list(zip(
        *self.self_ticket_serialization()))[1]
    ...
    if self.memory == 0:
      ...
      self.memories = [np.zeros(self.SERIALIZATION_LENGTH)
  ...
  def apply_history(self, update_ai=True):
    ...
    for turn in self.decision_turn_indexes:
      self.q_score_history.append(
          self.actual_scores[min(turn + self.q_lag, max_turn)])
    if update_ai:
  ...
  def record_history(self, serialization, ticket_serialization=None,
    ...
    if ticket_serialization != None:
      self.ticket_serialization_history.append(
          deepcopy(ticket_serialization))
    else:
      self.ticket_serialization_history.append(
          deepcopy(self.base_ticket_serialization[0]))
  ...
  def begin_selecting_tickets(self, min_tickets=1):
    ...
    ticket_selection = self.ai.decide_on_tickets(
        self, new_tickets, max(0, min_tickets - n_already_chosen_tickets))
    ticket_indexes = [t['index'] for t in ticket_selection]
    reject_tickets = [
        ticket for ticket in new_tickets if ticket['index'] not in ticket_indexes]
  ...
  def select_tickets(self, ticket_list, reject_tickets=[], max_top_of_deck_range=4):
    ...
    """
    max_top_of_deck_range = min(
        max_top_of_deck_range, len(self.game.tickets))
      ...
      for i in range(len(self.uncompleted_tickets))[::-1]:
        ...
        if owned_ticket['index'] == ticket['index']:
          self.completed_tickets.append(
              self.uncompleted_tickets.pop(i))
          self.negative_ticket_points -= ticket['points']
          self.positive_ticket_points += ticket['points']
  ...
  def build_tracks(self, segment, cost, color_id=0):
    ...
    for color, amount in cost.items():
      ...
      self.visible_trains[color] = max(
          0, self.visible_trains[color] - amount)
    ...
    self.game.serialized_board_state[2 * N_TRACKS *
                                     self.order + segment['index'] + color_id] = 1
    return 0
        ...
        if amount + n_locomotives >= length:
          ...
          for n_regular in range(lower_bound, upper_bound + 1):
            possibilities.append(
                {color: n_regular, 'locomotive': length - n_regular})
    else:
      amount = self.trains[color]
      if amount + n_locomotives >= length:
        lower_bound = max([0, length - n_locomotives])
  ...
  def determine_possible_tracks(self):
    ...
    return list(zip(track_segments, track_segment_costs))
  ...
  def update_points(self, check_longest_track=False):
    self.total_points = self.track_points + \
        self.positive_ticket_points - self.negative_ticket_points
  ...
  def append_serializations(self, serializations=None):
    ...
    player_serializations = [player.serialize_self(
        external=True) for player in self.other_players()]
    serializations_to_append = np.hstack(
        [game_serialization, self_serialization] + player_serializations)
  ...
  def serialize_self(self, external=False):
    ...
    for color, amount in self.visible_trains.items():
    ...
    if not external:
      for color, amount in self.trains.items():
  ...
  def self_ticket_serialization(self, new_tickets=None, min_tickets=None):
    ...
    if new_tickets == None:
      ...
      return list(zip(additional_ticket_combos,
    ...
    for num_new_tickets in range(min(min_tickets, n_new_tickets, ticket_limit - n_tickets), min(ticket_limit - n_tickets, n_new_tickets) + 1):
      ...
      for subset in combinations(new_tickets, num_new_tickets):
        ticket_combinations.append(
            self.uncompleted_tickets + list(subset))
...
def pad_with_np_zeros(serializations, limit=7):
  ...
  for serialization_set in serializations:
    if len(serialization_set) == limit:
      ...
      serialization_set += (limit - len(serialization_set)
                            ) * [np.zeros(N_CITIES + 1)]
...
def check_tracks_for_completion(tracks, ticket,
  ...
  valid_connections = [city for city in connecting_cities if frozenset(
      [starting_city, city]) in unexplored_paths]
...
def calculate_longest_track(tracks,
  ...
  if starting_city == None:
    ...
    for i, city in enumerate(valid_connections):
      ...
      elem_to_remove = frozenset([city, starting_city])
      unexplored_paths.remove(elem_to_remove)
      max_sublength = calculate_longest_track(
          tracks, unexplored_paths, starting_city=city)
      # second index of the CITY_CITY_CONNECTIONS object is the length (first is a numeric ID for the card)
      connection_lengths[i] = max_sublength + \
          CITY_CITY_CONNECTIONS[starting_city][city][1]
      unexplored_paths.add(elem_to_remove)
