...
def generate_all_points_images(
  ...
  Args:
    ...
    output_poath (str, optional): Where to save the numnber images. Defaults to "auto" (ask user for directory via a file dialog).
    relief (bool, optional): Whether to add a 3D effect to the numbers by adding edge highlights on one side and shadows on the other. Defaults to True.
  """
  # get current directory
  if output_path == "auto":
    tk.Tk().withdraw() # hide root window
    print("Select output directory where to save images in a (new) `points_images` folder. If no such folder exists, it will be created.")
    output_path: str = askdirectory(title="Select output directory where to save images in a (new) `points_images` folder.")
    if not output_path:
      print("aborted creation of points images. No output directory selected.")
      return
    print(f"Creating new `points_images` folder in {output_path} with subfolders:\n- `points_standard`,\n- `points_bonus`,\n- `points_penalty`.")
    output_path = os.path.join(output_path, "points_images")
  # current_dir = os.path.dirname(os.path.abspath(__file__))
  generate_points_images(
    save_filepath=os.path.join(output_path, "points_penalty"),
    font_size=font_size,
    font_name=font_name,
    font_path=font_path,
  ...
  generate_points_images(
    save_filepath=os.path.join(output_path, "points_standard"),
    font_size=font_size,
    font_name=font_name,
    font_path=font_path,
  ...
  generate_points_images(
    save_filepath=os.path.join(output_path, "points_bonus"),
    font_size=font_size,
    font_name=font_name,
    font_path=font_path,
...
def generate_points_images(
  ...
  for number in range(min_number, max_number + 1):
    ...
    # draw text
    text = str(number)
    _, _, *text_size = draw.textbbox((1,1), text, font=font, stroke_width=stroke_width)
...
def get_image_size(
  ...
  for number in range(min_number, max_number + 1):
    draw = ImageDraw.Draw(image)
    text = str(number)
    _, _, *text_size = draw.textbbox((0, 0), text, font=font, stroke_width=stroke_width)
    max_image_size = (
...
if __name__ == "__main__":
  ...
  generate_all_points_images(
    ...
    texture_path="assets\\points_images\\white_marble.jpg",
