...
class Graph_Editor_GUI:
  ...
  def show_edge_settings(self, particle_edge: Particle_Edge) -> None:
    ...
    image_override_filepath_var = self.add_node_image_setting(
        ...
        width=15)
    row_index += 1
    # add button to straighten connection
    straighten_connection_button = tk.Button(
        self.settings_frame,
        text="Straighten Connection",
        # cursor="hand2",
        command=lambda: self.straighten_connection(particle_edge))
    self.add_button_style(straighten_connection_button)
    straighten_connection_button.grid(
        row=row_index,
        column=0,
        sticky="new",
        columnspan=2,
  ...
  def straighten_connection(self, particle_edge: Particle_Edge) -> None:
    """
    Straighten the connection associated with the given edge.

    Args:
        particle_edge (Particle_Edge): one edge particle of the connection to straighten
    """
    location_1: str = particle_edge.location_1_name
    location_2: str = particle_edge.location_2_name
    connection_index: int = particle_edge.connection_index
    # find all edges between the two locations
    n_edges: int = self.particle_graph.get_connection_count(location_1, location_2)
    for connection_index in range(n_edges):
      # use particle_graph.straighten_connection()
      self.particle_graph.straighten_connection(
        location_1=location_1,
        location_2=location_2,
        connection_index=connection_index,
        ax=self.ax,
        x_periodic=False,
        y_periodic=False, # TODO: make periodicity adjustable
        max_connection_index=n_edges-1)
    self.canvas.draw_idle()
