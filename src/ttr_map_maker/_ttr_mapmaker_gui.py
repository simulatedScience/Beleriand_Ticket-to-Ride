...
from file_browsing import browse_image_file, browse_json_file, browse_txt_file, browse_directory, browse_ttf_file
...
class Board_Layout_GUI:
  ...
  def add_label_style(self, label: tk.Label, headline_level: int = 5, font_type: str = "normal"):
    ...
    label.configure(
      ...
      font=(self.gui_font, fontsize, font_type),
      )
  ...
  def add_button_style(self, button: tk.Button):
    button.configure(
      ...
      relief="flat",
      border=0,
      font=(self.gui_font, self.fontsize, "bold"),
  ...
  def add_entry_style(self, entry: tk.Entry, justify="right"):
    entry.configure(
      ...
      border=0,
      justify=justify,
      font=(self.gui_font, self.fontsize),
      )
  ...
  def draw_file_widgets(self, file_frame: tk.Frame):
    ...
    row_index += 1
    add_file_input(row_index, "Edge img Folder", self.edge_folder_path, lambda: browse_directory("browse edge images folder", self.edge_folder_path))
    row_index += 1
  ...
  def load_background_image(self) -> None:
    """
    Load the background image specified in the file input into the variable `self.background_image_mpl`.
    Update particle graph settings for background image.
    """
    if self.background_file.get() == "":
      print("No background image specified.")
      return
    # load background image
    try:
      print(f"Loading bg image from {self.background_file.get()}")
    ...
    self.particle_graph.set_bg_info(bg_image_path=self.background_file.get())
  ...
  def draw_toggle_widgets(self, toggle_frame: tk.Frame):
    ...
    repair_connection_ids_button.config(font=(self.gui_font, 8))
  ...
  def get_edge_color_map(self, edge_colors) -> dict:
    ...
    return {color: pkmn_colors.type_to_edge_image(
        pkmn_colors.color_to_energy_type(color), self.edge_folder_path.get()) for color in edge_colors}
  ...
  def scale_background_image(self, get_new_size: bool=True):
    ...
    # update project setup in particle graph
    self.particle_graph.set_bg_info(
        bg_image_size=(self.board_width.get(), self.board_height.get())
    )
    self.particle_graph.set_bg_info(
        bg_image_offset=(self.background_image_offset_x.get(), self.background_image_offset_y.get())
    )
  ...
  def draw_board_scaling_widgets(self, plot_param_frame: tk.Frame):
    ...
    def add_numeric_input(
        ...
        width: int = 5) -> Tuple[tk.Label, tk.Entry]:
      """
    ...
    graph_apperance_frame = tk.Frame(plot_param_frame)
    self.add_frame_style(graph_apperance_frame)
    graph_apperance_frame.grid(
        row=row_index,
        column=0,
        columnspan=5,
        sticky="nsew",
    ...
    board_size_widgets.append(graph_apperance_frame)
    # add Entry for node size
    add_numeric_input(graph_apperance_frame, row_index, column_index, "node size", self.node_size)
    column_index += 2
    # add Button to apply node size factor to all nodes
    apply_node_size_button = tk.Button(
        graph_apperance_frame,
        text="apply to nodes",
    ...
    row_index += 1
    column_index: int = 0
    # add Entries for label font and size
    # browser for label font file
    label_font_frame = tk.Frame(graph_apperance_frame)
    self.add_frame_style(label_font_frame)
    label_font_frame.grid(
        row=row_index,
        column=0,
        columnspan=5,
        sticky="nsew",
        padx=self.grid_pad_x,
        pady=self.grid_pad_y)
    board_size_widgets.append(label_font_frame)
    # populate label font frame with label, entry and file browser
    label = tk.Label(label_font_frame, text="label font", justify="left")
    self.add_label_style(label)
    label.grid(
        row=row_index,
        column=0,
        ...
        pady=(self.grid_pad_y, self.grid_pad_y))
    board_size_widgets.append(label)
    entry = tk.Entry(label_font_frame, textvariable=self.label_font, width=10)
    self.add_entry_style(entry)
    entry.xview_moveto(1)
    entry.grid(
        row=row_index,
    ...
    board_size_widgets.append(entry)
    browse_button = self.add_browse_button(
        frame=label_font_frame,
        row_index=row_index,
        column_index=2,
        command=lambda: browse_ttf_file("label font", self.label_font))
    board_size_widgets.append(browse_button)
    row_index += 1
    # add label size entry and button to apply label settings
    add_numeric_input(label_font_frame, row_index, column_index, "label size", self.label_fontsize)
    column_index += 2
    apply_label_settings_button = tk.Button(
        label_font_frame,
        text="apply to labels",
        command=self.apply_label_settings)
    self.add_button_style(apply_label_settings_button)
    apply_label_settings_button.grid(
        row=row_index,
        column=column_index,
        sticky="nsew",
        padx=(self.grid_pad_x, self.grid_pad_x),
        pady=(0, self.grid_pad_y))
    board_size_widgets.append(apply_label_settings_button)
  ...
  def apply_label_settings(self):
    """
    Applies the label font and size to all labels.
    """
    if self.particle_graph is not None:
      self.particle_graph.set_label_settings(self.ax, self.label_font.get(), self.label_fontsize.get())
    # update plot
    self.canvas.draw_idle()
  ...
  def toggle_task_export_mode(self):
    ...
    if not self.gui_mode.get().lower() == "task export":
      self.task_export_ui.save_settings()
