...
def generate_counting_strip(
    ...
    num_cells = max_number - min_number + 1
    ...
    for i, num in enumerate(range(min_number, max_number + 1)):
        ...
        # Resize the number image, use `number_offset` to move number relative to background
        num_img_resized = num_img.resize((new_width, new_height))
    ...
    strip.save(f"assets\\counting_strip_{min_number}-{max_number}_{length_px}.png")
